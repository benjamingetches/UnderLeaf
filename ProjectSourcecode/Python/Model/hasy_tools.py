...
def load_images(csv_filepath, symbol_id2index, one_hot=True, flatten=False):
    ...
    for i, data_item in enumerate(data):
        ...
        if img.ndim == 3 and img.shape[2] == 3:
            img = np.mean(img, axis=2).astype(np.uint8)  # Convert to grayscale by averaging channels
        ...
        if flatten:
            ...
            images[i, :, :, 0] = img  # Add depth dimension

        symbol_id = data_item['symbol_id']
        if symbol_id in symbol_id2index:
            label = symbol_id2index[symbol_id]
            labels.append(label)
        else:
            print(f"Warning: {symbol_id} not found in symbol_id2index")  # Debugging line
